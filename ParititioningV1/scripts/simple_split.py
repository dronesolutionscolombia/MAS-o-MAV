...
from mavros_msgs.srv import CommandBool, SetMode, CommandTOL, CommandHome, WaypointPush, WaypointPull, WaypointClear
...
filename = "/Users/Liss/MEGA/DecompositionCodes/geometry_decom/guacas_3_google_cropped.tif"
...
def detected_uavs():
    ...
    #EL 10 es el mayor numero de UAV para detecta
    k = 0
    while k<10:
        name = '/uav'+str(k+1)+'/mavros/state'
        for i in topics:
            if i[0] == name:
                n_uavs+=1
        k+=1
...
def allocate_mission(cols, uav_paths):
	lt = []
	lg = []
	n_uavs = detected_uavs()
	print "uavs-->"+ str(n_uavs)
	...
	for i in range(0, n_uavs):
		...
		print("wp:"+str(len(mission)))

		upload_mission = rospy.ServiceProxy('/uav'+str(i+1)+'/mavros/mission/push', WaypointPush)
		read_mission = rospy.ServiceProxy('/uav'+str(i+1)+'/mavros/mission/pull', WaypointPull)
		clear_mission = rospy.ServiceProxy('/uav'+str(i+1)+'/mavros/mission/clear', WaypointClear)

		start_index = 0
		print(clear_mission())
		print(upload_mission(start_index, mission))
		print(read_mission())
...
##allocate_mission(cols, uav_paths)
...
def arming_and_takeoff(i):
	rospy.init_node('TEST')
	global sub
	sub = None
	
	arming = rospy.ServiceProxy('/uav'+str(i)+'/mavros/cmd/arming', CommandBool)
	takeoff = rospy.ServiceProxy('/uav'+str(i)+'/mavros/cmd/takeoff', CommandTOL)
	land = rospy.ServiceProxy('/uav'+str(i)+'/mavros/cmd/land', CommandTOL)
	sub = rospy.Subscriber('/uav'+str(i)+'/mavros/global_position/global', NavSatFix, pos_uav)
	mode = rospy.ServiceProxy('/uav'+str(i)+'/mavros/set_mode', SetMode)
	home = rospy.ServiceProxy('/uav'+str(i)+'/mavros/cmd/set_home', CommandHome)
	read_mission = rospy.ServiceProxy('/uav'+str(i)+'/mavros/mission/pull', WaypointPull)

	print("arming .. "+ str(arming(True)))
	rospy.sleep(5.0)
	print(curr_pose.latitude, curr_pose.longitude)
	print(read_mission())
	print(home(False, curr_pose.latitude, curr_pose.longitude, curr_pose.altitude))
	takeoff(0,0, curr_pose.latitude, curr_pose.longitude, 5)
	rospy.sleep(2.0)

#for i in range(n_uavs):

	
#	arming_and_takeoff(i+1)
#	rospy.sleep(2.0)

#for i in range(n_uavs):


#	mode = rospy.ServiceProxy('/uav'+str(i+1)+'/mavros/set_mode', SetMode)
#	print("Auto .. "+ str(mode(220, '')))
#	rospy.sleep(5.0)
