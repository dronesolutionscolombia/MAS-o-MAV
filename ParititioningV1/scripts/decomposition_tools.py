...
def new_voronoicentroids(labelarea, centroids, final):
	...
	"""for p in range(0,len(labelarea)):
		if labelarea[p][0] == 'C':
			min_x = labelarea[p][4]
			min_y = labelarea[p][5]
			c_point = (labelarea[p][4], labelarea[p][5])
			newPoints.append((labelarea[p][4] + delta, labelarea[p][5] - delta))
	...
	for p in range(len(labelarea)):
		if labelarea[p][0] == 'Qmin' or labelarea[p][0] == 'Q':
			q_point = (labelarea[p][4], labelarea[p][5])
			d = distance(c_point, q_point)
			delta = d//2
			if labelarea[p][4] - min_x >= 0 and labelarea[p][5] - min_y >= 0:
				newPoints.append((labelarea[p][4] + delta, labelarea[p][5] + delta))
			elif labelarea[p][4] - min_x >= 0 and labelarea[p][5] - min_y < 0:
				newPoints.append((labelarea[p][4] + delta, labelarea[p][5] - delta))
			elif labelarea[p][4] - min_x < 0 and labelarea[p][5] - min_y < 0:
				newPoints.append((labelarea[p][4] - delta, labelarea[p][5] - delta))
			elif labelarea[p][4] - min_x < 0 and labelarea[p][5] - min_y >= 0:
...
def name_C(centroids, labelarea, final, rows, cols):
	...
	#print "\nC Labeled: \n" + str(labelarea) + "\n"
	return labelarea, labelarea[n][4],labelarea[n][5]
