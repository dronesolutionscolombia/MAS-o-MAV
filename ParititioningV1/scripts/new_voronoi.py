...
"""for i in range(rows):
	for j in range(cols):
		...
		if (i >= center_y-r and i <= center_y+r):
			if (j==center_x+r or j==center_x-r):
				Points.append((j,i))"""
...
"""for i in  range(0, len(Points)):
	x, y = Points[i] 
	cv2.circle(_image, (x,y), 3, (0, 250, 0), 1, 8)"""
...
for p in randPoints:
	for n in range(0, len(color)):
		if (final.item(p[0],p[1],0) == color[n][0]) and (final.item(p[0], p[1],1) == color[n][1]) and (final.item(p[0], p[1],2) == color[n][2]):
			labelarea[s] = labelarea[s][:4] + (p[0], p[1])
			s = s + 1
...
Points2 = []
circle = np.zeros(((r_x-l_x),(b_y-u_y)), np.uint8)
cv2.circle(circle, (center_y, center_x), radio/2, 255)
Points2 = np.transpose(np.where(circle==255))
